...
class Penjualan(models.Model):
    ...
    #     )
    #     return super(Penjualan, self).get_empty_list_help(help)
    #
    # @api.model
    ...
    order_line = fields.One2many('penjualan.detail', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    def _compute_access_url(self):
        super(Penjualan, self)._compute_access_url()
        for order in self:
    ...
    def _compute_type_name(self):
        for record in self:
            record.type_name = _('Quotation') if record.state in ('draft', 'sent', 'cancel') else _('Penjualan')
    ...
    def _track_subtype(self, init_values):
        ...
        return super(Penjualan, self)._track_subtype(init_values)
        ...
        if vals.get('name', _('New')) == _('New'):
            ...
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code(
                    'penjualan.penjualan', sequence_date=seq_date) or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('penjualan.penjualan', sequence_date=seq_date) or _('New')
    ...
    def _write(self, values):
        ...
        if self.env.context.get('mail_activity_automation_skip'):
            return super(Penjualan, self)._write(values)
    ...
    def copy_data(self, default=None):
        ...
        return super(Penjualan, self).copy_data(default)
    ...
    def name_get(self):
        if self._context.get('sale_show_partner_name'):
            ...
            return res
        return super(Penjualan, self).name_get()
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(Penjualan, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def _get_invoiceable_lines(self, final=False):
        ...
        return self.env['penjualan.detail'].browse(invoiceable_line_ids)
    ...
    def action_quotation_send(self):
        ...
        if template.lang:
            lang = template._render_template(template.lang, 'penjualan.penjualan', self.ids[0])
        ctx = {
            'default_model': 'penjualan.penjualan',
    ...
    def message_post(self, **kwargs):
        ...
        return super(Penjualan, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)
...
class PenjualanDetail(models.Model):
    ...
    def write(self, values):
        ...
        result = super(PenjualanDetail, self).write(values)
        return result
    ...
    order_id = fields.Many2one('penjualan.penjualan', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(PenjualanDetail, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def unlink(self):
        ...
        return super(PenjualanDetail, self).unlink()
