...
class OrderPembelian(models.Model):
    _name = "order.pembelian"
    _inherit = ['mail.thread', 'mail.activity.mixin', 'portal.mixin']
    _description = "Order Pembelian"
    ...
    order_line = fields.One2many('order.pembelian.detail', 'order_id', string='Order Lines',
    ...
    def _compute_access_url(self):
        super(OrderPembelian, self)._compute_access_url()
        for order in self:
    ...
    def create(self, vals):
        ...
        if vals.get('name', 'New') == 'New':
            ...
            vals['name'] = self.env['ir.sequence'].with_context(force_company=company_id).next_by_code('order.pembelian',
                                                                                                       sequence_date=seq_date) or '/'
        return super(OrderPembelian, self.with_context(company_id=company_id)).create(vals)

    def write(self, vals):
        res = super(OrderPembelian, self).write(vals)
    ...
    def unlink(self):
        ...
        return super(OrderPembelian, self).unlink()

    def copy(self, default=None):
        ctx = dict(self.env.context)
        ctx.pop('default_product_id', None)
        self = self.with_context(ctx)
        new_po = super(OrderPembelian, self).copy(default=default)
    ...
    def _track_subtype(self, init_values):
        ...
        return super(OrderPembelian, self)._track_subtype(init_values)
    ...
    def message_post(self, **kwargs):
        ...
        return super(OrderPembelian, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)
    ...
    def action_invoice_register_payment(self):
        ...
        return {
            'name': _('Pay'),
            'res_model': 'jewellery_management.pembayaran.wizard',
            'view_mode': 'form',
            'view_id': self.env.ref(
                'jewellery_management.view_pembayaran_wizard').id,
            'context': {'default_amount_total': self.amount_total},
            'view_mode':'form',
            'target': 'new',
            'type': 'ir.actions.act_window',
        }
...
class OrderPembelianDetail(models.Model):
    _name = 'order.pembelian.detail'
    ...
    @api.depends('nilaimurni', 'order_id.currency_id')
    ...
    order_id = fields.Many2one('order.pembelian', string='Order Reference', index=True, required=True,
                               ondelete='cascade')
    ...
    def create(self, values):
        ...
        line = super(OrderPembelianDetail, self).create(values)
    ...
    def write(self, values):
        ...
        return super(OrderPembelianDetail, self).write(values)

    def unlink(self):
        for line in self:
            if line.order_id.state in ['purchase', 'done']:
                raise UserError(_('Cannot delete a purchase order line which is in state \'%s\'.') % (line.state,))
        return super(OrderPembelianDetail, self).unlink()
    ...
    def _product_id_change(self):
        ...
        # self.price_unit = self.order_id.currency_id.rate * self.weight
    ...
    def _create_account_move_line(self, credit_account_id, debit_account_id, journal_id, qty, description, tarif):
        self.ensure_one()
        AccountMove = self.env['account.move'].with_context(default_journal_id=journal_id)

        move_lines = self._prepare_account_move_line(qty, credit_account_id, debit_account_id, description, tarif)
        if move_lines:
    ...
    def _account_entry_move(self, qty, description, svl_id, cost):
        ...
        self.ensure_one()
        self.with_context(force_company=self.company_id)._create_account_move_line(self.accoun, acc_valuation, journal_id, qty, description, cost,tarif)
        #journal_id, acc_src, acc_dest, acc_valuation = self._get_accounting_data_for_valuation()
        #self._create_accout_move_line(acc_dest, acc_src, journal_id, qty, description, self.tarif)
